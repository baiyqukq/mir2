...
type
   ...
   private
      ...
      repeattime: longword;  //0x98 반복 애니메이션 시간 (-1: 계속)
...
begin
   ...
   if (not FixedEffect) then begin
      ...
      if TargetActor <> nil then begin
         ms := GetTickCount - m_dwFrameTime;  //이전 효과를 그린후 얼마나 시간이 흘렀는지?
         m_dwFrameTime := GetTickCount;
         //TargetX, TargetY 재설정
       ...
       //  Dir16 := GetFlyDirection16 (OldFlyX, OldFlyY, FlyX, FlyY);
         ...
         ms := GetTickCount - m_dwFrameTime;  //효과의 시작후 얼마나 시간이 흘렀는지?
...
begin
   ...
   //TargetCret := TActor(target);//瞳놔君唐훙痰踞샙裂잚珂，쉥?阮쳐엽?
end;
