...
var
   ...
   dline, temp: string;
const
   BOXWIDTH = (SCREENWIDTH div 2 - 214) * 2{374}; //41 ¡ƒÃÏøÚŒƒ◊÷øÌ∂»
begin
   len := Length (str);
   temp := '';
   i := 1;
		...
		with MSurface do begin
			...
			with PlayScene do begin
				for k:=0 to m_ActorList.Count-1 do begin
					...
					if g_boShowRedHPLable then actor.m_boOpenHealth:=True; //œ‘ æ—™Ãı
         ...
         //∏ ¿« ªÛ≈¬ «•Ω√ (¿”Ω√ «•Ω√)
			if (g_nAreaStateValue and $04) <> 0 then begin
				BoldTextOut (MSurface, 0, 0, clWhite, clBlack, 'π•≥««¯”Ú');
			end;
