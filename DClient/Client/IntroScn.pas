...
type
   ...
   TSelChar = record
      ...
      Selected: Boolean;
      FreezeState: Boolean; //TRUE:얼은상태 FALSE:녹은상태
      Unfreezing: Boolean; //녹고 있는 상태인가?
...
begin
   ...
   if Length(m_EdNewId.Text) < 3 then begin
      FrmDlg.DMessageDlg ('되쩌琅뵀돨낀똑극伎댕黨3貫.', [mbOk]);
      Beep;
      m_EdNewId.SetFocus;
      Result := FALSE;
   end;
   ...
   if Key = #13 then begin
      ...
      if Sender = m_EdNewPasswd then begin
         if Length(m_EdNewPasswd.Text) < 4 then begin
            FrmDlg.DMessageDlg ('쵱쯤낀똑극伎댕黨 4貫.', [mbOk]);
            Beep;
      ...
      if Sender = m_EdConfirm then begin
         if m_EdNewPasswd.Text <> m_EdConfirm.Text then begin
            FrmDlg.DMessageDlg ('랗늴渴흙돨쵱쯤꼇寧逞！！！', [mbOk]);
            Beep;
      ...
      if (Sender = m_EdSSNo) and (not EnglishVersion) then begin  //한국인 경우.. 주민등록번호 간략 채크
...
begin
   ...
   with FrmMain do begin  //login
      ...
      if EnglishVersion then  //영문버전은 주민등록번호 입력을 안한다.
...
begin
   ...
   m_boUpdateAccountMode := TRUE; //기존에 있는 정보를 재입력하는 경우
   NewIdRetry (TRUE);
...
begin
   ...
   if not EnglishVersion then begin //영문 버전에서는 체크안함
      if not NewIdCheckSSNo then
         exit;
   end;
...
begin
   ...
   if chrname <> '' then begin
      ...
      FrmMain.SendNewChr (FrmMain.LoginId, chrname, shair, sjob, ssex); //새 캐릭터를 만든다.
   end;
end;
   ...
   for n:=0 to 1 do begin
      if ChrArr[n].Valid then begin
		 ...
		 if ChrArr[n].Unfreezing then begin 
            ...
            if not ChrArr[n].Selected and (not ChrArr[n].FreezeState and not ChrArr[n].Freezing) then begin //선택되지 않았는데 녹아있으면
