...
if (GetTickCount - m_dwSpellTick) > self.m_nNextHitTime  then begin
        m_dwSpellTick:=GetTickCount();
...
begin
 ...
 if m_TargetCret = nil then begin
 if m_wStatusTimeArr[STATE_TRANSPARENT{0x70}]=0 then begin
...
time1:=-1;
 ...
 if m_TargetCret <> nil then begin
      m_nTargetX:=m_TargetCret.m_nCurrX;
     m_nTargetY:=m_TargetCret.m_nCurrY;
  if (abs(m_nTargetX-m_nCurrX) = 2) and (abs(m_nTargety-m_nCurry)=2) then begin
     if time1=0 then begin //do drag back on random
