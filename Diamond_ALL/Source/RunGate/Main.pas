    ...
    try
      if (GetTickCount - dwRefConsoleMsgTick) >= 1000 then begin
        ...
        if not boShowBite then begin
          ...
          LabelBufferOfM2Size.Caption:='BufferM2: ' +IntToStr(nBufferOfM2Size  div 1024) + ' KB';
          LabelProcessMsgSize.Caption:='PMsg: ' +IntToStr(nProcessMsgSize  div 1024) + ' KB';
          LabelLogonMsgSize.Caption:='LMsg: ' +IntToStr(nHumLogonMsgSize  div 1024) + ' KB';
          LabelPlayMsgSize.Caption:='PMsg: ' +IntToStr(nHumPlayMsgSize  div 1024) + ' KB';
          LabelDeCodeMsgSize.Caption:='DeCode: ' +IntToStr(nDeCodeMsgSize  div 1024) + ' KB';
          LabelSendBlockSize.Caption:='BMsg: ' +IntToStr(nSendBlockSize  div 1024) + ' KB';
          {
          ...
          LabelReviceMsgSize.Caption:='RMsg: ' + IntToStr(nReviceMsgSize) + ' B';
          LabelBufferOfM2Size.Caption:='BufferM2: ' +IntToStr(nBufferOfM2Size) + ' B';
          ...
          LabelLogonMsgSize.Caption:='LMsg: ' +IntToStr(nHumLogonMsgSize) + ' B';
          LabelPlayMsgSize.Caption:='PSize: ' +IntToStr(nHumPlayMsgSize) + ' B';
          LabelDeCodeMsgSize.Caption:='DeCode: ' +IntToStr(nDeCodeMsgSize) + ' B';
...
//        if (GetTickCount - dwCheckServerTick) > 60 * 1000 then begin
    ...
    if (GetTickCount - dwRefConsolMsgTick) > 1000 then begin
      ...
      LabelReviceLimitTime.Caption:='RecvLimit: ' + IntToStr(dwProcessReviceMsgTimeLimit);
      LabelSendLimitTime.Caption:='SendLimit: ' + IntToStr(dwProcessSendMsgTimeLimit);
      LabelReceTime.Caption:='RecvTime: ' + IntToStr(dwProcessClientMsgTime);
      LabelSendTime.Caption:='SendTime: ' + IntToStr(dwProcessServerMsgTime);
      {
...
begin
  try
//    AddMainLogMsg('欢迎使用翎风系列游戏软件...',0);
//    AddMainLogMsg('网站:http://www.gameofmir.com',0);
//    AddMainLogMsg('论坛:http://bbs.gameofmir.com',0);
    AddMainLogMsg('Service started...',2);
    boServiceStart:=True;
    boGateReady:=False;
...
var
  nSockIdx:Integer;
begin
  AddMainLogMsg('Service stopped...',2);
  ...
  ServerSocket.Close;
  ClientSocket.Close;
  AddMainLogMsg('Socket Closed...',2);
end;

procedure TFrmMain.MENU_CONTROL_STOPClick(Sender: TObject);
begin
  if Application.MessageBox('Do you want to stop the server?',
                            'Confirm',
...
begin
  AddMainLogMsg('Loading Config...',3);
